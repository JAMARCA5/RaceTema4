...
```{r}
library(dplyr)  
cran <- as_tibble(mydf) # El data frame se guarda en un tibble
rm(mydf)
print(cran)  
```
...
glimpse(cran)#Para ver la estrucutura de los datos
```
...
```{r}
descargas_plotly<-cran%>%filter(package =="plotly"&r_version == "3.5.1")# Usamos filter y el parámetro r_version
head(descargas_plotly)
```
6. Elige aquellas descargas realizadas desde España (ES), con versiones de R superiores a la 3.4.0
```{r}
descargas_plotly2<-cran%>%filter(country=="ES"&r_version>"3.4.0")
(descargas_plotly2)
```

7. Elige aquellas descargas realizadas desde España (ES), o Portugal (PT).
```{r}
es_por<-cran%>%filter(country=="ES"|country=="PT")
...
```{r}
bytes100500_linux<-cran%>%filter(size>1000500&r_os=="linux-gnu")
bytes100500_linux
```
9. Elige aquellas descargas en las que la versión del sistema operativo no está vacía.
```{r}
no_vacia<-cran %>% filter(!is.na(r_os) & r_os != "")
head(no_vacia)
```
...
```{r}
cran2ascd<-cran2%>%arrange(ip_id)
head(cran2ascd)
```
12. Ordena `cran2` en orden descendente según la variable `ip_id`.
```{r}
cran2desc<-cran2%>%arrange(desc(ip_id))
head(cran2desc)
```
12. Ordena `cran2` según los valores de `package e ip_id`.
```{r}
cran2_package_ip_id<-cran2%>%arrange(package,ip_id)
head(cran2_package_ip_id)
str(cran2)

```
13. Ordena `cran2` según los valores de `country` (ascendente), `r_version` (desdencente) e `ip_id`(ascendente).
```{r}
cran2_country_r_version_ip_id<-cran2%>%arrange(country,desc(r_version),desc(ip_id))
head(cran2_country_r_version_ip_id)
```
...
```{r}
cran3 <- cran3 %>%mutate(size_mb = size / 2^20)  # Convertir bytes a MB
head(cran3)
```
...
```{r}
cran3<-cran3%>%mutate(size_gb=size_mb/2^20) 
cran3
```
...
```{r}
cran3 <- cran3 %>%
  mutate(correct_size = size + 1000)  # Corregir el tamaño sumando 1000 bytes
...
pack_sum <- by_package %>%
  summarise(
    count = n(),                        # Número de descargas por paquete
    unique = n_distinct(ip_id),         # Cantidad de IPs únicas
    countries = n_distinct(country),    # Cantidad de países únicos
    med_bytes = stats::median(size, na.rm = TRUE)  # Mediana del tamaño de descargas
  )
head(pack_sum)
```
...
```{r}
top_counts <- cran %>%
  group_by(package) %>%                              # Agrupar por paquete
  summarise(count = n()) %>%                         # Contar descargas por paquete
  filter(count > quantile(count, probs = 0.99)) %>%  # Filtrar el 1% superior
  arrange(desc(count))     
head(top_counts)
...
top_counts_sorted <- cran %>%
  group_by(package) %>%                              # Agrupar por paquete
  summarise(count = n()) %>%                         # Contar descargas por paquete
  ungroup() %>%                                      # Eliminar agrupamiento
  filter(count > quantile(count, probs = 0.99)) %>%  # Filtrar el 1% superior
  arrange(desc(count))                               # Ordenar descendentemente por count
head(top_counts_sorted)
```
...
top_unique_sorted <- cran %>%
  group_by(package) %>%                              # Agrupar por paquete
  summarise(unique = n_distinct(ip_id)) %>%         # Contar ordenadores únicos por paquete
  ungroup() %>%                                      # Eliminar agrupamiento
  filter(unique > quantile(unique, probs = 0.99)) %>% # Filtrar el 1% superior
  arrange(desc(unique))                              # Ordenar descendentemente por unique
```
...
```{r}
cran %>%select(ip_id, country, package, size) %>%mutate(size_mb = size / 2^20) %>%  # Convertir bytes a megabytes
  print()
```
...
```{r}
cran %>%select(ip_id, country, package, size) %>%mutate(size_mb = size / 2^20) %>%  # Convertir bytes a megabytes
  filter(size_mb <= 0.5) %>%         # Filtrar descargas ≤ 0.5 MB
  print()
```
...
cran %>%select(ip_id, country, package, size) %>%mutate(size_mb = size / 2^20) %>%  # Convertir bytes a megabytes
  filter(size_mb <= 0.5) %>%         # Filtrar descargas ≤ 0.5 MB
  arrange(desc(size_mb)) %>%         # Ordenar en orden descendente
  print()
```
...
# Transformar el data frame a formato tidy
students_tidy <- students %>%pivot_longer(cols = c(male, female),  # Convertir columnas "male" y "female" en filas
               names_to = "sex",        # Nueva columna para el tipo de sexo
               values_to = "count") %>% # Nueva columna para la cantidad
  mutate(sex = recode(sex, male = "M", female = "F"))  # Opcional: Reemplazar nombres
print(students_tidy)
```
...
students2_tidy <- students2 %>%
  pivot_longer(cols = starts_with("male") | starts_with("female"), # Unir columnas de género
               names_to = "sex_class",    # Nueva columna con sexo y grupo
               values_to = "count") %>%   # Nueva columna con los valores
  separate(sex_class, into = c("sex", "class"), sep = "_") %>% # Separar en dos columnas
  mutate(sex = recode(sex, male = "M", female = "F")) # Opcional: cambiar nombres
...
students3_tidy <- students3 %>%
  pivot_longer(cols = starts_with("class"),  # Seleccionar columnas que comienzan con "class"
               names_to = "class_exam",      
               values_to = "score") %>%  
  drop_na(score) %>%  # Eliminar valores NA
  separate(class_exam, into = c("class", "exam"), sep = "_") %>% 
...
students3_tidy <- students3 %>%
  ...
  drop_na(score) %>%  
  separate(class_exam, into = c("class", "exam"), sep = "_") %>% 
  mutate(class = parse_number(class)) %>%  # Convertimos "classX" en solo el número X
  pivot_wider(names_from = exam, values_from = score)
...
student_info <- students4 %>%
  select(id, name, sex,) %>%   # Seleccionar solo info del estudiante
  distinct()  # Eliminar duplicados
gradebook <- students4 %>%
  select(id, class, midterm, final)  # Datos de calificaciones
print(student_info)
print(gradebook)
```
...
```{r}
sat_tidy <- sat %>%
  filter(score_range != "Total") %>%  # Eliminar la fila de totales
  pivot_longer(
    cols = -score_range,  # Mantener 'score_range' fija
    names_to = c("part", "sex"),  # Separar nombres de columnas en 'part' y 'sex'
    names_sep = "_" ,  # Separador que indica la división
    values_to = "count"  # Nuevo nombre para los valores
  )
print(sat_tidy)
```
...
sat_grouped <- sat_tidy %>%
  group_by(part, sex) %>%  # Agrupar por 'part' y 'sex'
  mutate(total = sum(count),   # Total de estudiantes en cada grupo
         prop = count / total) # Proporción dentro del grupo
print(sat_grouped)
```
...
library(ggplot2)
# Crear gráfico
ggplot(sat_grouped, aes(x = score_range, y = prop, fill = sex)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ part) +  # Facetas por tipo de prueba
  labs(title = "Distribución de calificaciones en el SAT por sexo",
...
# Obtener el día de la semana (como número y como nombre en inglés)
weekday_number <- wday(this_day, week_start = 1)  # Lunes = 1, Domingo = 7
weekday_name <- wday(this_day, label = TRUE, week_start = 1)  # Nombre en inglés
# Mostrar los valores extraídos
print(paste("Año:", year_value))
print(paste("Mes:", month_value))
print(paste("Día:", day_value))
print(paste("Día de la semana (número):", weekday_number))
print(paste("Día de la semana (nombre):", weekday_name))
```
...
```{r}
library(lubridate)
# Importar fechas con diferentes formatos
my_date <- ymd("2019-02-14")
print(my_date)
print(class(my_date))  # Ver la clase del objeto
# Importar "1989 May 17"
date1 <- ymd("1989 May 17")
print(date1)
# Importar "March 12, 1975"
...
```{r}
# Importar 25081985 en formato día, mes, año (ddmmyyyy)
fecha1 <- dmy("25081985")
print(fecha1)  # Resultado esperado: "1985-08-25"
# Importar 192012 en formato año, mes, día (yymmdd)
fecha2 <- ymd("192012")  # Error probable por ambigüedad
#Agregar separadores:
fecha2_corregida <- ymd("1920-12-01")  # Interpretando como "1920-12-01"
print(fecha2_corregida)
```
...
```{r}
# Almacenar la cadena con fecha y hora
dt1 <- "2014-08-23 17:23:02"
dt1_corregida<- ymd_hms(dt1)
print(dt1_corregida)  # Resultado: "2014-08-23 17:23:02 UTC"
# Importar solo la hora
dt2 <- "03:22:14"
dt2_corregida<- hms(dt2)
print(dt2_corregida)  # Resultado: "3H 22M 14S"
```
43. Importa el vector de fechas almacenado el `dt2`
```{r}
dt2 <- c("2023-05-12 14:23:45", "2021-07-19 08:15:30", "2019-12-31 23:59:59")
# Convertir el vector de cadenas en formato fecha-hora
dt2_corregida <- ymd_hms(dt2)
```
...
```{r}
this_moment <- now()
# Actualizar la hora, minutos y segundos con los valores actuales
this_moment_updated <- update(this_moment, 
                              hours = hour(now()), 
                              minutes = minute(now()), 
                              seconds = second(now()))
print(this_moment_updated)
```
...
```{r}
# 1. Generar la fecha actual en la zona horaria de Nueva York
nyc <- now(tz = "America/New_York")
print(nyc)

# 2. Añadir dos días a la fecha actual para obtener la fecha de salida
salida <- nyc + days(2)
print(salida)

# 3. Actualizar la variable `salida` con la hora de salida (17:34)
salida <- update(salida, hours = 17, minutes = 34, seconds = 0)
print(salida)

# 4. Calcular la hora de llegada en referencia a la zona horaria de Nueva York
llegada_nyc <- salida + hours(15) + minutes(50)
print(llegada_nyc)

# 5. Convertir la hora de llegada a la zona horaria de Hong Kong
llegada_hk <- with_tz(llegada_nyc, tz = "Asia/Hong_Kong")
print(llegada_hk)

# 6. Fecha de la última vez que viste a tu amigo en Singapur
ultimo_encuentro <- ymd("2008-06-17", tz = "Asia/Singapore")

# 7. Crear el intervalo de tiempo entre la última vez y la llegada a Hong Kong
intervalo <- interval(ultimo_encuentro, llegada_hk)

# 8. Convertir el intervalo a un período legible
